...
function buildMessage(order: DeliveryOrder, portalUrl: string) {
  ...
  const gift = guest?.gift && typeof guest.gift === "object"
    ? guest.gift as Record<string, unknown>
    : null;
  ...
  const amount = order.payment_status === "paid"
    ? "Pagado"
    : `Cobrar $${Number(order.total_amount || 0).toLocaleString("es-CO")}`;
...
export function DeliveryDispatchBridge() {
  ...
  useEffect(() => {
    const onSubmit = async (event: SubmitEvent) => {
      ...
      if (!orderId || !siteId || (!partner && !reference)) return;
      ...
      for (let attempt = 0; attempt < 8; attempt += 1) {
        await new Promise((resolve) => window.setTimeout(resolve, attempt === 0 ? 450 : 350));
        ...
        const candidate = row as DeliveryOrder | null;
        const saved = candidate
          && (!partner || candidate.dispatch_partner === partner)
          && (!reference || candidate.dispatch_reference === reference);
        if (saved) {
          order = candidate;
          break;
        }
      ...
      if (!order) {
        ...
        setNotice({ tone: "error", text: lastError || "La asignación no quedó guardada. No se generó el enlace." });
        return;
      }

      const { data: linkData, error: linkError } = await supabase.rpc(
        "create_order_delivery_courier_link",
        { p_order_id: orderId, p_site_id: siteId },
      );
  ...
  return (
    <div className={`fixed bottom-4 right-4 z-[200] max-w-sm rounded-2xl border px-4 py-3 text-sm font-bold shadow-xl ${notice.tone === "ok" ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-red-200 bg-red-50 text-red-900"}`}>
      <button
        ...
        className="float-right ml-3 text-current/60"
        aria-label="Cerrar"
      >
        ×
      </button>
